...
namespace TestServer.Controllers
{
	...
	public class PaymentController : ControllerBase
	{
		...
		public async Task<IActionResult> PaymentCallbackVnpay()
		{
			...
			{
				return StatusCode(500, new { Message = "Error processing VNPAY callback.", Detail = ex.Message, Response = response });
			}
		...
		{
			if (vehicleMonthId <= 0) return BadRequest(new { Success = false, Message = "Invalid vehicleMonthId." });

			var vpm = await _db.VehiclePerMonths.FirstOrDefaultAsync(v => v.VehicleMonthId == vehicleMonthId);
			if (vpm == null) return NotFound(new { Success = false, Message = "VehiclePerMonth not found." });
		...
		public async Task<IActionResult> PayFromUnity([FromBody] UnityPaymentRequest request)
		{
			if (request == null || request.VehicleMonthId <= 0)
				return BadRequest(new UnityPaymentResponse { Success = false, Message = "Invalid request or VehicleMonthId." });
			...
			if (vpm == null)
				return NotFound(new UnityPaymentResponse { Success = false, Message = $"VehiclePerMonth id {request.VehicleMonthId} not found." });

			// Calculate outstanding amount
			var due = Math.Round((double)(vpm.TotalCost - vpm.AmountPaid), 2);

			if (due <= 0)
				return BadRequest(new UnityPaymentResponse { Success = false, Message = "No outstanding amount to pay.", Amount = 0 });
			...
			if (request.Simulate)
			{
				...
				return Ok(new UnityPaymentResponse
				{
					Success = true,
			...
			var paymentModel = new PaymentInformationModel
			{
				...
				OrderDescription = $"Payment for VehicleMonth {vpm.VehicleMonthId} - VehicleId {vpm.VehicleId}"
			};
